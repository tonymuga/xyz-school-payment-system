...
public class StudentController {
    ...
    @ApiResponses(value = {
            @io.swagger.v3.oas.annotations.responses.ApiResponse(responseCode = "200", description = "Student is valid and enrolled",
                    content = @Content(mediaType = "application/json",
                            schema = @Schema(implementation = io.swagger.v3.oas.annotations.responses.ApiResponse.class))),
            @io.swagger.v3.oas.annotations.responses.ApiResponse(responseCode = "404", description = "Student not found or not enrolled",
                    content = @Content(mediaType = "application/json"))
    })
    ...
    public ResponseEntity<?> validateStudent(@RequestBody StudentRequest studentRequest) {
        ...
        if (isEnrolled) {
            return ResponseEntity.ok(new ApiResponse("Success", true, "Student is valid and enrolled."));
        }
        return ResponseEntity.status(HttpStatus.NOT_FOUND)
                .body(new ApiResponse("Failure", false, "Student not found or not enrolled."));
    }
}
